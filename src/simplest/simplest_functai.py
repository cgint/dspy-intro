...
import mlflow
mlflow.set_experiment("simplest_functai")
mlflow.autolog()
...
def main():
    ...
    with mlflow.start_run(run_name="simplest_functai_joke_for_john"):
        the_joke=joke_for_john()
        print(f"\n\n{the_joke}")
    
    with mlflow.start_run(run_name="simplest_functai_joke_for_john_funnyness_factor"):
